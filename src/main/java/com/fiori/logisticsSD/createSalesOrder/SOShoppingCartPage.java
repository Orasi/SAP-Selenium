...
public class SOShoppingCartPage extends BasePage{
    ...
    public void updateItemQuantity(String itemName, String quantity){
    	int itemRow = tblItems.getRowWithCellText(itemName, 2);
    	new TextboxImpl(tblItems.getCell(itemRow, 4).findElement(By.tagName("input"))).set(quantity);
    }
    
    public void updateItemDelivery(String itemName, String daysOut){
    	int itemRow = tblItems.getRowWithCellText(itemName, 2);
    	new TextboxImpl(tblItems.getCell(itemRow, 5).findElement(By.tagName("input"))).set(DateTimeConversion.getDaysOut(daysOut, "MMM dd, yyyy"));
    }
    

    public void removeItem(String itemName){
    	int itemRow = tblItems.getRowWithCellText(itemName, 2);
    	new ButtonImpl(tblItems.getCell(itemRow, 6).findElement(By.tagName("button"))).click();
    }
}
